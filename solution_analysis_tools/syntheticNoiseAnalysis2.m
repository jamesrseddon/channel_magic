function [] = syntheticNoiseAnalysis2(result_set,T,delta,N,T_N)
%SYNTHETICNOISEANALYSIS2 Analyse results from synthetic noise sweep.
...
figure
plot(lambda_vector,dyad_neg_vector,'.-');
xlabel(lambda_label);
ylabel(dyadic_label);
...
figure
plot(lambda_vector,product_vec,'.-');
xlabel(lambda_label);
ylabel(dyadic_label);
...
dyad_neg_max_single = dyad_neg_vector(1);
const_norm_single = 1/(dyad_neg_max_single);
...
for kk = 1:M
    ...
    Delta_norm(kk) = (lambda - 1) + lambda*dyad_neg*const_norm_single;
end

figure
plot(lambda_vector,Delta,'.-g');
xlabel(lambda_label);
ylabel('Total error bound');
title(plot_title);

plot_title = ['Single copy normalised'];
figure
plot(lambda_vector,Delta_norm,'.-g');
xlabel(lambda_label);
ylabel('Total error bound');
title(plot_title);
...
const_norm = 1/(dyad_neg_max);


figure
semilogy(lambda_vector,product_vec,'.-');
xlabel(lambda_label);
ylabel(dyadic_label);
...
figure
semilogy(lambda_vector,Delta_tot,'.-g');
xlabel(lambda_label);
ylabel('Total error bound');
title(['Many copies, N = ' num2str(N) ', number of samples = ' num2str(T_N)]);

figure
semilogy(lambda_vector,Delta_tot_norm,'.-g');
xlabel(lambda_label);
ylabel('Total error bound');
